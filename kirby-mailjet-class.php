...
class KirbyMailjet {
		...
		*/
		...
		if($response->success() && count($response->getData()) > 0) {
			...
			foreach ($campaigns as $campaign) {
				if($campaign['Subject'] != a::get($campaign_body, 'Subject', '')) continue;
				if($campaign['ContactsListID'] != $contactslistID) continue;
				if($segid = a::get($campaign_body, 'SegmentationID', null)) {
	...
	public static function loadTranslation() {
		if(self::$_translationLoaded) return;
